...
function Records({ records, setRecords, projects, showToast }: { records: RecordItem[]; setRecords: React.Dispatch<React.SetStateAction<RecordItem[]>>; projects: Project[]; showToast: (message: string) => void }) {
  ...
  return (
    <div className="page"><header className="page-header"><div><Eyebrow>DOCUMENT CONTROL</Eyebrow><h1>Records</h1><p>Search, preview and download every project file from one place.</p></div></header>
      ...
      {preview && <div className="modal-backdrop" onMouseDown={() => setPreview(null)}><div className="preview-modal" onMouseDown={(e) => e.stopPropagation()}><header><span className="file-badge">PDF</span><div><strong>{preview.name}</strong><small>{preview.project} · {preview.stage}</small></div><button onClick={() => setPreview(null)} aria-label="Close preview"><X size={19} /></button></header><div className="mock-document"><FileText size={54} /><strong>Document preview</strong><p>This local preview represents the future Supabase Storage document viewer.</p></div></div></div>}
    </div>
  );
}
